...
int MotFlechi::handicap()
{
    ...
    if (debog) qDebug()<<"        motflechi::handicap"<<this->morpho();
    int ret = 0;
...
void MotFlechi::lance()
{
    ...
    for (int i=0;i<_phrase->nbRegles();++i)
    {
        ...
        if (debog) qDebug()<<"  lance"<<morpho()<<"  règle"<<r->id()<<"estSub"<<r->estSub(this);
...
Requete* MotFlechi::lanceReqSup(Regle* r)
{
    ...
    _phrase->ajRequete(nr);
    _lreqSup.append(nr);
    if (debog) qDebug()<<morpho()<<"lanceReqSup"<<nr->debog();
    nr->ajHist("\n------------------\nRequête numéro "+nr->numc());
    nr->ajHist(nr->humain());
    nr->ajHist(nr->debog());
    return nr;
...
QString MotFlechi::traduction()
{
    return "MotFlechi::traduction() à écrire";
}

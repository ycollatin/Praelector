...
/*
 *  AGENDA 
 *  pluriel de monseigneur ?
 *  saillir -> sailli
 *  accoucher, déménager : être ET avoir !
 *  p.s. et pp : le v. simple seoir;
 *  adj de couleur et composés : find ("-")

 *  participe prés du pronominal la réplication ne prend que "se"
 *  verbes en -ayer : donner les deux possibilités
 *  le pluriel de grand-mère a deux solutions.
...
QString mode[8] = {" ", "indicatif", "subjonctif", "conditionnel", "impératif",
...
QStringList AuxEtre = QStringList() <<"accourir"<<"advenir"<<"aller"<<"arriver"
<<"décéder"<<"devenir"<<"échoir"<<"enlaidir"<<"entrer,"<<"intervenir"<<"monter"
<<"mourir"<<"naître"<<"obvenir"<<"partir"<<"parvenir"<<"provenir"<<"redevenir"
<<"rentrer"<<"reparaître"<<"repartir"<<"ressortir"<<"rester"<<"retomber"
<<"retourner"<<"revenir"<<"sortir"<<"survenir"<<"tomber"<<"venir";

QStringList Eter = QStringList() << "acheter"<<"racheter"<<"bégueter"<<"corseter"
<<"crocheter"<<"fileter"<<"fureter"<<"haleter"<<"celer"<<"déceler"<<"receler"
<<"ciseler"<<"démanteler"<<"écarteler"<<"encasteler"<<"geler"<<"dégeler"<<"congeler"
<<"surgeler"<<"marteler"<<"modeler"<<"peler";

QStringList intransitifs = QStringList() <<"aboutir"<<"agir"<<"appartenir"<<"bavarder"<<"boiter"
<<"bondir"<<"bourdonner"<<"briller"<<"circuler"<<"consister"<<"contribuer"<<"daigner"
<<"déjeuner"<<"déplaire"<<"dîner"<<"durer"<<"errer"<<"étinceler"<<"exister"<<"falloir"<<"frémir"<<"frissonner"
<<"gambader"<<"grelotter"<<"grincer"<<"hésiter"<<"insister"<<"jouir"<<"luire"<<"lutter"<<"marcher"<<"mentir"
<<"obéir"<<"paître"<<"parler"<<"paraître"<<"plaire"<<"pleuvoir"<<"pouvoir"<<"procéder"<<"profiter"
<<"réagir"<<"résonner"<<"resplendir"<<"ressembler"<<"retentir"<<"rire"<<"ronronner"<<"rougir"<<"ruisseler"
<<"sembler"<<"sévir"<<"souper"<<"sourire"<<"suffire"<<"tarder"<<"tinter"<<"tournoyer"<<"tressaillir"
<<"triompher"<<"voltiger"<<"voyager";
...
{
	if (inf.startsWith ("s'") || inf.startsWith("se "))
		return true;
	/*
	inf.insert (0, 1, ','); inf.push_back (',');	
	if (npremieres (inf, 2) == "s'" || npremieres (inf, 3) == "se ") return true;
	*/
...
QString Verbe::circonflexe()
{
	...
	QString d;
	if (der == "a") d = "â";
	else if (der == "i" || der == "î") d = "î";
	else if (der == "u" || der == "û") d = "û";
	else d = "";
	result = otedernieres (result, 1);
	result.append (d);
	/*
...
QString Verbe::conjugue (int P, int T, int M, int V, bool pr, int g, int n) 
{
	...
	if (V>1 && pas_de_passif (inf) && M != part) return ""; // pas de passif;
	...
	if (V == actif)
	{
		...
		{
			if (T == pres) result = IndPres(P);
			else if (T == fut) result = IndFut(P);
			else if (T == impf) result = IndImpf(P);
			else if (T == psimple) result = IndPs(P);
			else result = compose(aux, P, T - 4, ind, actif);
		}
		else if (M == subj) // subjonctif actif
		{
			if (T == pres) result = SubjPres(P);
			else if (T == impf)
				if (!RadPs ().isEmpty ())
				{
					if (P == 3)
						result = circonflexe() + "t";
					else result = IndPs(2)+"s"+GetDesSubjPres(P);
				}
				else result = "";
			else if (T == pcompose)  // 5
				result = compose(aux, P, pres, subj, actif);
			else if (T == pqp) result = compose(aux,P,impf,subj,actif);
			else result = "";
		}
		else if (M == cond)
		{
		...
		{
			if (T == pres) result = inf;
			else if (T == pcompose) 
				result = compose(aux, P, pres, infin, actif);
		}
		else if (M == part)
		{
			if (T == pres) result = PartPres();
			else if (T == pcompose)
	...
	else if (V == passif)
	{
		...
		if (M == part)
		{
			switch (T)
			{
				case pres: return PartPres();
				case psimple: 
						   {
						   	   QString res = PP();
						   	   if (g == 2) res.append ("e");
							   if (n == 2 && (!res.endsWith("s"))) 
							   	   res.append ("s");
							   return res;
						   }
				default:break;
	...
	if (pr) result = Pron(P, result, pronominal);
...
QString TVeter::RD (QString R, QString D)
{
	...
	if (desR) p = R.lastIndexOf ('e', R.length()-3);
	else p = R.lastIndexOf('e', R.length()-2);
	// transformer en è si l''initiale de D est e 
	if ((D.startsWith ('e') && (!D.endsWith ('z'))) || desR) 
		R.replace (p, 1, "è");
	return R + D;
}
...
QString TVettell::RD (QString R, QString D)
{
	...
	// repérer la dernière occurence de e dans le radical
	if (desR) p = R.lastIndexOf ('e', R.length()-3);
	else p = R.lastIndexOf ('e', R.length()-1);
	// transformer en è si l''initiale de D est e 
...
QString TVebrer::RD(QString R, QString D)
{
	...
	// transformer en è si l''initiale de D est e.
	if (p < string::npos && D[0] == 'e' && (!D.endsWith ('z') || R.endsWith ('r')))
		R.replace (p, 1, "è");
	return R + D;
}
...
QString TVyer::RD(QString R, QString D)
{
	...
	if (D[0] == 'e' && !D.endsWith ('z'))
		R = otedernieres (R, 1) + "i";
	return R + D;
}
...
{
	QString D[7] = {"", "faux", "faux", "faut", "faillons", "faillez", "faillent"};
	return D[P];
}
...
{
	if (P < 4) return otedernieres (inf, 3);
	else return otedernieres (inf, 4) + "gn";
}
...
{
	if (P == 3 || P == 6)
		return Verbe::conjugue(P, T, M, V, P, Pr);
	return "";
}
...
QString Verbe::compose( QString A, int P, int T, int M, int V)
{
	...
	{
		Verbe * aux;
		if (A == "avoir") aux = new TVavoir ("avoir");
		else aux = new TVetre ("être");
...
Verbe * verbe_m (QString inf)
{
	...
	else if (IsLast("naître", inf) and inf.size () < 8) // != connaître 
		Vb = new TVnaitre(inf);
...
QString Nom::pluriel()
{
	const QStringList als = QStringList ()
		<< "bal" << "cal" << "carnaval" << "chacal" 
		<< "festival" << "récital" << "régal";

	if (sing == "oeil" || sing == "œil") return "yeux";
	else if (sing == "ail") return "aulx";
	return sing + "s";
}
...
Nom * nom_m (QString n)
{
	QString als[11] = {"bal", "cal", "carnaval", "cérémonial", 
		"chacal", "festival", "pal", "récital", "régal", "santal"};
	QString ail[7] = {"bail", "corail", "émail", "soupirail", "travail", "vantail", "vitrail"};
	...
	if (QString("sxz").contains (d)) nom = new NomSXZ (n);
	else if (deuxder (n) == "al" && index_t (als, n, 11) < 0) nom = new NomAL (n);
	else if (n == "bétail") result = inex;
	else if (dernieres (n, 3) == "ail" && index_t (ail, n, 6) > -1) nom = new NomAIL (n);
	else if ((IsLast("eu", n) || IsLast("au", n)) && index_t (aueus, n, 5) < 0) 
		nom = new NomAUEU(n);
	else if (index_t (oux, n, 6) > -1) nom = new NomAUEU (n);//result = n + "x";
	else nom = new Nom (n);
	return nom;
}
...
{
	QString mascs[13] =
    {"bénin","doux","faux","favori","fou","frais","jaloux","long","malin","mou",
     "muet","roux","sot"} ;
	QString fems[13] =
	{"bénigne","douce","fausse","favorite","folle","fraîche","jalouse","longue",
	 "maligne","molle","muette","rousse","sotte"};
...
{
	Nom * nom;
	if (fem) nom = new Nom (feminin ());
	else nom = nom_m (graphie);
	QString result = nom->pluriel ();
	return result;
}

QString Adjectif::accorde(int g, int n)
{
	QString ret;
	if (g == 1) ret = feminin();
	else ret = graphie;
...
QString Al::pluriel (bool fem)
{
	...
	QString liste_al[7] = {"banal","bancal","fatal","final","glacial", "natal","naval"};
...
QString Et::feminin()
{
	QString liste[10] = {"complet","concret","désuet","discret","incomplet",
		"indiscret","inquiet","quiet","replet", "secret"};
...
QString Accorde(QString adj, int g, int n)
{
	...
	if (IsLast("el", adj) || IsLast("eil", adj) || adj == "gentil" || adj== "nul")
		...
		inst = new Gras(adj);
	else if ((QStringList() <<"mon"<<"ton"<<"son").contains (adj))
		inst = new Mon (adj);
